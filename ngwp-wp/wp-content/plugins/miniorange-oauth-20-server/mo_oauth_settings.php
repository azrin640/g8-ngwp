...
* Description: Use WordPress as your OAuth 2.0 Server. Supports Single Sign On and other OAuth 2.0 features.
* Version: 2.6.1
* Author: miniOrange
...
class mo_oauth_server {
	...
	function tutorial($page) {
		...
		$manager = new PointersManager( $file, '2.5.1', 'custom_admin_pointers' );
		// print_r($manager);
		$manager->parse();
	...
	function miniorange_oauth_save_settings(){
		...
		if ( isset( $_POST['option'] ) and $_POST['option'] == "mo_oauth_mo_server_message" ) {
		...
		if(isset($_POST['option']) and $_POST['option'] == "mo_oauth_register"){
			update_option('goto_registration', true);
			return;
		}
		else if(isset($_POST['option']) and $_POST['option'] == "mo_oauth_validate_otp"){
			if( mo_oauth_server_is_curl_installed() == 0 ) {
				return $this->mo_oauth_show_curl_error();
			}
			//validation and sanitization
		...
		if( isset( $_POST['option'] ) and $_POST['option'] == "mo_oauth_verify_customer" ) {	//register the admin to miniOrange
		...
		else if( isset( $_POST['option'] ) and $_POST['option'] == "mo_oauth_add_client" ) {
			$clientid = '';
			$clientsecret = '';
			if($this->mo_oauth_check_empty_or_null($_POST['mo_oauth_custom_client_name']) || $this->mo_oauth_check_empty_or_null($_POST['mo_oauth_client_redirect_url'])) {
				...
				$client_name = stripslashes( $_POST['mo_oauth_custom_client_name'] );
				$redirect_url = stripslashes( $_POST['mo_oauth_client_redirect_url'] );
				$moOauthServerDb = new MoOauthServerDb();
				$clientlist = $moOauthServerDb->get_clients();
				if(sizeof($clientlist) < 1) {
					$moOauthServerDb->add_client($client_name, moosGenerateRandomString(30), $redirect_url);
					update_option( 'message', 'Your settings are saved successfully.' );
					$this->mo_oauth_show_success_message();
				}
				else {
					update_option( 'message', 'You can add only 1 client with free version.' );
					$this->mo_oauth_show_error_message();
				}
			}
		}
		else if( isset( $_POST['option'] ) and $_POST['option'] == "mo_oauth_update_client" ) {
			$clientid = '';
			$clientsecret = '';
			if($this->mo_oauth_check_empty_or_null($_POST['mo_oauth_custom_client_name']) || $this->mo_oauth_check_empty_or_null($_POST['mo_oauth_client_redirect_url'])) {
				update_option( 'message', 'Please enter valid Client Name and Redirect URI.');
		...
		} else if(isset($_POST['option']) and $_POST['option'] == "mo_oauth_server_master_switch") {
		...
		} else if(isset($_POST['option']) and $_POST['option'] == "mo_oauth_general_settings") {
			if(isset($_POST['mo_server_token_length'])) {
				update_option('mo_oauth_server_token_length', (int) stripslashes($_POST["mo_server_token_length"]));
			}
			...
			$this->mo_oauth_show_success_message();
			wp_redirect('admin.php?page=mo_oauth_server_settings&tab=general_settings');
		} else if(isset($_POST['option']) and $_POST['option'] == "mo_oauth_server_enforce_state") {
			...
			$this->mo_oauth_show_success_message();
			wp_redirect('admin.php?page=mo_oauth_server_settings&tab=general_settings');
		} else if(isset($_POST['option']) and $_POST['option'] == "mo_oauth_server_enable_oidc") {
			...
			update_option( 'message', 'Your settings are saved successfully.' );
			$this->mo_oauth_show_success_message();
			wp_redirect('admin.php?page=mo_oauth_server_settings&tab=advanced_settings');
		}
		//submit google form
		...
		elseif( isset( $_POST['option'] ) and $_POST['option'] == "mo_oauth_contact_us_query_option" ) {
			...
			// Contact Us query
			$email = stripslashes($_POST['mo_oauth_contact_us_email']);
		...
		} else if( isset( $_POST['option'] ) and $_POST['option'] == "mo_oauth_change_phone" ) {
			update_option('mo_oauth_registration_status','');
		}
		if ( isset( $_POST['option'] ) and $_POST['option'] == 'mo_oauth_server_skip_feedback' ) {
